...
static void sKeyCallback(GLFWwindow* window, int key, int scancode, int action, int mods) {
	...
	if (action == GLFW_PRESS) {
		...
		case GLFW_KEY_SPACE: // Launch a ball
			...
			break;
     /*   case GLFW_KEY_X: // Launch a ball
			if (app) {app->m_creature.m_bodies[TRONC] = app->m_creature.m_bodies[TRONC]->GetWorldCenter()+ b2Vec2(0.0f,5.0f);}
			break;*/
...
static void sInterface() {
	...
	if (ui.showMenu) {
        ...
        ImGui::Checkbox("Supprimer Tete", &settings.deleteHead);
